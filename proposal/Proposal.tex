...
\title{Demonstrating a Standardized IBM Methodology using Chinook Salmon PSAT Data from the Gulf of Alaska}
...
\begin{objective}
\textbf{Standardized Decision Models} Movement (and more generally decision) modeling is fundamentally about modeling the selection of outcomes. So, given a set of choices $v_k$ and a set of features $n_k$ describing those choices, we want a standardized method to fit, evaluate, and use models of the form:
...
\begin{objective}
\textbf{Standardized Decision Simulations} Given an individual in a specific situation, the decision models described above produce a set of probabilities for each potential next outcome $v_k$. However there is nothing preventing us from taking each of those outcomes, considering them each a new situation, and applying the models again. Carried out repeatedly we will end up with a probabilistic diffusion of the original individual across its decision space. Furthermore, if we can do this for one individual we can do it for an entire distribution of individuals. In sum, using this diffusion modeling approach we can simulate the movement through a decision space of an entire population. 

Our second objective then is to build such a diffusion model. However, we would like to do it in such a way that life history, mortality, and environmental models can be provided as input as well. This diffusion model will then allow us to explore the spatio-temporal behavior of a stock under both nominal conditions as well as under various exploratory management strategies. 
...
\textbf{Choices:} Discrete depth ranges. \newline
...
\textbf{Choices:} Spatial grid cell selection as movement. Also if clearly different modes of movement are found (migration vs foraging for example) we'll predict the mode as well. \newline
...
In building these models our primary purpose is to demonstrate the tooling exists to prudently build movement models. We can frame this goal in terms of a "standard" machine learning pipeline and work-flow. 
...
\item Diagnoses of Fit and Comparison of Different Models
\item Identifying Sample Groups that are More or Less Fit
\item Exploring Discovered Patterns
\end{enumerate}
...
The outcome will be a package in Python for building, diagnosing, and running movement models. Note that at this point the purpose is to demonstrate how to build movement models for the purpose of building simulations. It will be in the next chapter that specific research outcomes directly relevant to management will manifest themselves. 

\subsection{Chapter 2}

Chapter 2 will be concerned with taking these movement models for Chinook salmon in the Gulf of Alaska and building a diffusion model. We will then demonstrate its capabilities by exploring a series of management questions. It is worth noting that these questions have been chosen based on their facility in demonstrating the aspects of the simulation as opposed to their priority to current Chinook salmon management in the GOA.  \newline

The overview of the components of this diffusion model are outlined in Fig. \ref{fig:simulation}. 
...
\begin{figure}[h!] 
  ...
  \medskip
	\small
	Boxes with rounded edges would be inputs specified by the user. The hexagons are functions 		provided by the diffusion model and the trapezoids are data generated during the main loop. It is important to note that in this design we are planning for the most data rich case. However, depending on the application, mortality or environmental data may in fact have quite limited availability.
  \label{fig:simulation}
\end{figure}


The specific questions we would like to answer with this simulation are:
